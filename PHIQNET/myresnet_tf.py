...
class Conv2dWithName(nn.Module):
    def __init__(self,in_planes, out_planes, kernel_size=3, stride=1,
                     padding=0, groups=1, bias=True, dilation=1,name=None):
...
def conv1x1(in_planes, out_planes, stride=1,name=None):
    """1x1 convolution"""
    return Conv2dWithName(in_planes, out_planes, kernel_size=1, stride=stride, bias=False,name=name)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0],name='conv3')
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1],name='conv4')
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2,
                                       dilate=replace_stride_with_dilation[2],name='conv5')
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False,name=None):
        ...
        for lyer in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups=self.groups,
                                ...
                                norm_layer=norm_layer,name=name+'_block%d'%(lyer+1)))

        return nn.Sequential(*layers)

    def init_from_tf(self,tf_model):
        for m in self.modules():
            if isinstance(m, (Conv2dWithName,BatchNorm2dWithName)):
                layer=tf_model.get_layer(m.name)
                m.set_weight(layer)
                '''
        with torch.no_grad():
            layer=tf_model.get_layer('final_fc')
            weights=layer.get_weights()
            weight=torch.from_numpy(weights[0]).transpose(0,1)
            self.final_fc.weight.copy_(weight)
            bias=weights[1]
            bias=torch.from_numpy(bias)
            self.final_fc.bias.copy_(bias)
            '''
